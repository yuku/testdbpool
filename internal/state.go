...
func CreateDatabase(ctx context.Context, db *sql.DB, dbName, templateName string) error {
	...
	// PostgreSQL automatically handles template database locking when using CREATE DATABASE WITH TEMPLATE
	// We don't need to terminate connections - PostgreSQL will wait if the template is in use
